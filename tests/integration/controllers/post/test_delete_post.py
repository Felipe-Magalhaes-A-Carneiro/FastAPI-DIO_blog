...
async def test_delete_posts_success(client: AsyncClient, access_token: str):
    # Given
...
async def test_delete_post_not_found_fail(client: AsyncClient, acess_token: str):
    # Given
    headers = {"Authorization": f"Bearer {acess_token}"}
    post_id = 4

    # When
    response = await client.patch(f"/posts/{post_id}", headers= headers)
